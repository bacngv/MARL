...
class Runner(object):
    ...
    def save_to_csv(self, main_loss, main_reward, iteration):
    ...
    def run(self, variant_eps, iteration, win_cnt=None):
        ...
        max_nums, nums, mean_rewards, total_rewards, train_loss, render_list = self.play(
            env=self.env, 
            n_round=iteration, 
            ...
            print_every=50, 
            eps=variant_eps, 
            render=(iteration + 1) % self.render_every == 0 if self.render_every and self.render_every > 0 else False, 
            train=self.train, 
            cuda=self.cuda
        )
        ...
        if self.train:
            ...
            # save main training log
            self.save_to_csv(train_loss, info['main']['total_reward'], iteration)
