...
class IQL(base.ValueNet):
    def __init__(self, env, name, handle, sub_len, memory_size=2**10, batch_size=64, 
                 update_every=5, use_mf=False, learning_rate=0.0001, 
                 polyak_tau=0.005, gamma=0.95, expectile=0.7):
        super().__init__(env, name, handle, update_every=update_every, use_mf=use_mf, 
                         learning_rate=learning_rate, polyak_tau=polyak_tau, 
                         gamma=gamma, expectile=expectile)
    ...
    def train(self, cuda):
        ...
        for i in range(batch_num):
            ...
            obs = torch.FloatTensor(obs).permute([0, 3, 1, 2]).to(device)
            obs_next = torch.FloatTensor(obs_next).permute([0, 3, 1, 2]).to(device)
            feat = torch.FloatTensor(feat).to(device)
            feat_next = torch.FloatTensor(feat_next).to(device)
            acts = torch.LongTensor(acts).to(device)
            rewards = torch.FloatTensor(rewards).to(device)
            dones = torch.FloatTensor(dones).to(device)
            masks = torch.FloatTensor(masks).to(device)
            
            loss, info = self.train_step(obs=obs, feature=feat, obs_next=obs_next, 
                                         feature_next=feat_next, dones=dones, rewards=rewards, 
                                         acts=acts, prob=None, mask=masks)
            self.update()
            total_loss += loss
            if i % 50 == 0:
                print('[*] IQL Loss:', loss, '/ Info:', info)
        print('[*] IQL Total Loss:', total_loss)
        return total_loss

    def save(self, dir_path, step=0):
        os.makedirs(dir_path, exist_ok=True)
        eval_file_path = os.path.join(dir_path, "iql_eval_{}".format(step))
        target_file_path = os.path.join(dir_path, "iql_target_{}".format(step))
