...
class ActorCritic(nn.Module):
    ...
    def train(self, cuda):
        ...
        ct = 0
        gamma = self.gamma
        # collect episodes từ nhiều buffer thành 1 buffer liên tục
        for episode in batch_data:
        ...
        self.optim.step()
        print('[*] PG_LOSS:', np.round(pg_loss.detach().cpu().item(), 6),
              '/ VF_LOSS:', np.round(vf_loss.detach().cpu().item(), 6),
              '/ ENT_LOSS:', np.round(neg_entropy.detach().cpu().item(), 6),
              '/ Value:', np.mean(value.detach().cpu().numpy()))
        print('[*] AC TOTAL_LOSS:', np.round(total_loss.detach().cpu().item(), 6))
...
class MFAC(nn.Module):
    ...
    def train(self, cuda):
        ...
        print('[*] PG_LOSS:', np.round(pg_loss.detach().cpu().item(), 6),
              ...
              '/ ENT_LOSS:', np.round(neg_entropy.detach().cpu().item(), 6),
              '/ Value:', np.mean(value.detach().cpu().numpy()))
        print('[*] MFAC TOTAL_LOSS:', np.round(total_loss.detach().cpu().item(), 6))
        return total_loss.item()   
    def act(self, **kwargs):
        flatten_view = kwargs['obs'].reshape(kwargs['obs'].size()[0], -1)
        h_view = F.relu(self.net['obs_linear'](flatten_view))
